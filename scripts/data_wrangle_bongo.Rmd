...
In addition, some lines of code were taken from a script written by Joanne Breckenridge's to ensure consistency in the way taxa were recorded. Please note however, that in her script she further standardizes the taxonomic observations to facilitate comparison across vessels. In this script, we use the highest granularity available, where accurate.
...
bongo_fnl <- bongo_fnl %>% 
  ...
  mutate(geodeticDatum = "WGS84",
         ...
         bibliographicCitation = "Hunt, B., Mahara, N., & Pakhomov, E. (2023). Zooplankton Bongo Net Data from the 2019 and 2020 Gulf of Alaska International Year of the Salmon Expeditions (v1.0). North Pacific Anadromous Fish Commission. https://doi.org/10.21966/7cmt-ca72",
...
The following section is per Joanne Breckenridge to improve the estimates for the abundance data associated with certain observed taxa, and to add the lifestage for Euphausiids after Joanne consulted with Natalie Mahara. Credits for the code go to Joanne.
...
#where KagLeg$species = C1, C2, or C3...
bongo_abun$stage[bongo_abun$species == 'C1'] <- 'C1'
bongo_abun$stage[bongo_abun$species == 'C2'] <- 'C2'
bongo_abun$stage[bongo_abun$species == 'C3'] <- 'C3'

#Euphausiid stage is often missing, after consulting with Natalie...

bongo_abun$stage[bongo_abun$id == 'Euphausia pacifica' & bongo_abun$sex == 'F'] <- 'adult'
bongo_abun$stage[bongo_abun$id == 'Euphausia pacifica' & bongo_abun$sex == 'M'] <- 'adult'
bongo_abun <- bongo_abun %>% 
  mutate(stage = replace(stage, is.na(stage) & id == 'Euphausia pacifica', 'juvenile'))

bongo_abun$stage[bongo_abun$id == 'Thysanoessa longipes' & bongo_abun$sex == 'F'] <- 'adult'
bongo_abun$stage[bongo_abun$id == 'Thysanoessa longipes' & bongo_abun$sex == 'M'] <- 'adult'
bongo_abun <- bongo_abun %>% 
  mutate(stage = replace(stage, is.na(stage) & id =='Thysanoessa longipes', 'juvenile'))

bongo_abun$stage[bongo_abun$id == 'Thysanoessa inspinata' & bongo_abun$sex == 'F'] <- 'adult'
bongo_abun$stage[bongo_abun$id == 'Thysanoessa inspinata' & bongo_abun$sex == 'M'] <- 'adult'
bongo_abun <- bongo_abun %>% 
  mutate(stage = replace(stage, is.na(stage) & id =='Thysanoessa inspinata', 'juvenile'))

bongo_abun$stage[bongo_abun$id == 'Thysanoessa spinifera' & bongo_abun$sex == 'F'] <- 'adult'
bongo_abun$stage[bongo_abun$id == 'Thysanoessa spinifera' & bongo_abun$sex == 'M'] <- 'adult'
bongo_abun <- bongo_abun %>% 
  mutate(stage = replace(stage, is.na(stage) & id =='Thysanoessa spinifera', 'juvenile'))

bongo_abun$stage[bongo_abun$id == 'Thysanoessa inermis' & bongo_abun$sex == 'F'] <- 'adult'
bongo_abun$stage[bongo_abun$id == 'Thysanoessa inermis' & bongo_abun$sex == 'M'] <- 'adult'
bongo_abun <- bongo_abun %>% 
  mutate(stage = replace(stage, is.na(stage) & id =='Thysanoessa inermis', 'juvenile'))

bongo_abun$stage[bongo_abun$id == 'Tessarabrachion occulatum' & bongo_abun$sex == 'F'] <- 'adult'
bongo_abun$stage[bongo_abun$id == 'Tessarabrachion occulatum' & bongo_abun$sex == 'M'] <- 'adult'
bongo_abun <- bongo_abun %>% 
  mutate(stage = replace(stage, is.na(stage) & id =='Tessarabrachion occulatum', 'juvenile'))

bongo_abun$stage[bongo_abun$id == 'Stylocheiron maximum' & bongo_abun$sex == 'F'] <- 'adult'
bongo_abun$stage[bongo_abun$id == 'Stylocheiron maximum' & bongo_abun$sex == 'M'] <- 'adult'
bongo_abun <- bongo_abun %>% 
  mutate(stage = replace(stage, is.na(stage) & id =='Stylocheiron maximum', 'juvenile'))

bongo_abun$stage[bongo_abun$id == 'Nematocelis difficilis' & bongo_abun$sex == 'F'] <- 'adult'

# Some additional standardization as per Joanne Breckenridge, or after consultation with Alexei Pinchuk/Moira Galbraith.
# Please note, that I am not doing any lumping of taxa here, however, if at a later stage this was revised that revision is included. 
# The previous identification of the taxa will remain recorded under previousIdentifications:
bongo_abun <- bongo_abun %>%
  mutate(previousIdentifications = case_when(
    bongo_abun$id == "Primno pacifica" ~ "Primno pacifica",
    bongo_abun$id == "Scaphocalanus brevicaudatus" ~ "Scaphocalanus brevicaudatus", 
    bongo_abun$id == "Lucicutia sp." ~ "Lucicutia sp.",
    bongo_abun$id == "Aglantha sp." ~ "Aglantha sp.",
    bongo_abun$id == "Scolecithricella sp." & bongo_abun$stage %in% c("C5") ~ "Scolecithricella sp.",
    bongo_abun$id == "Mesocalanus sp." ~ "Mesocalanus sp.",
    bongo_abun$id == "Racovitzanus sp." ~ "Racovitzanus sp.",
    bongo_abun$id == "Ctenocalanus sp." ~ "Ctenocalanus sp."))
...
bongo_abun$id[bongo_abun$id == "Lucicutia sp." ] <- "Lucicutia flavicornis"
bongo_abun$id[bongo_abun$id == "Aglantha sp." ] <- "Aglantha digitale"
bongo_abun$id[bongo_abun$id == "Scolecithricella sp." & bongo_abun$stage %in% c("C5")] <- "Scolecithricella minor"
bongo_abun$id[bongo_abun$id == "Mesocalanus sp." ] <- "Mesocalanus tenuicornis"
bongo_abun$id[bongo_abun$id == "Racovitzanus sp." ] <- "Racovitzanus antarcticus"
bongo_abun$id[bongo_abun$id == "Ctenocalanus sp." ] <- "Ctenocalanus vanus"
...
bongo_abun <- bongo_abun %>%
  group_by(eventID, vessel, station, id, sf_proc, stage, sex, previousIdentifications) %>%
...
bongo_abun$scientificname <- gsub("Gaetanus intermedius", "Gaetanus simplex", bongo_abun$scientificname)
...
# Confirm all names in our original data table have an associated AphiaID.
...
# In the occurrence data file there is no field to include abundance. This will get included in the eMOF. 
...
bongo_sampling <- bongo %>%
  select(eventID,
         tow_distance,
         volume_filtered = sampleSizeValue) %>%
  mutate(speed_tow = 1) %>%
  distinct(eventID, .keep_all = TRUE) %>% 
...
bongo_emof <- bongo_abun %>%
  select(eventID, occurrenceID, lifeStage, sex, abundance) %>%
  mutate_all(as.character) %>%
  ...
  mutate(measurementID = paste(occurrenceID, measurementType, sep = "-"),
    ...
    measurementUnit = case_when(
      ...
      measurementType != "abundance" ~ "not applicable"),
    measurementUnitID = case_when(
