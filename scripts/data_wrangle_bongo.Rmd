...
bongo_abun <- read_csv(here("original_data", "IYSZoopAbund2019_2020Long.csv")) %>%
  janitor::clean_names() %>%
  select(-x1)
...
bongo_kag <- read_csv(here("original_data", "IYSKaganoSpecRemovedfromNet.csv")) %>%
  janitor::clean_names() 
bongo_kag$previous_identifications <- NA

bongo_abun <- rbind(bongo_abun, bongo_kag)

bongo_abun$date <- as.Date(bongo_abun$date, format = "%m/%d/%Y")
bongo_abun$net <- 1
...
bongo_abun$scientificname <- gsub("Scaphocalanus brevicaudatus", "Scaphocalanus brevicornis", bongo_abun$scientificname)
...
``` {r, eval = FALSE}
bongo_abun <- bongo_abun %>% select(-genus)
...
bongo_occ <- bongo_abun %>% select(-c(individualCount, ind_total, abund_m3_flow_est, abund_m3_depth_est))
