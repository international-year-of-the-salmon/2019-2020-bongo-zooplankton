...
bongo <- read_excel(here("original_data", "IYS cruise logbook_2019.xlsx"),
...
bongo <- bongo %>%
  filter(net %in% c("1", "ND")) %>%
  filter(preservation == "formalin") %>%
  mutate(eventID = paste("IYS2019", "Kaganovsky", station, net, sep = "-")) %>%
  distinct()
...
Dataframes are created for each unique `eventID` layer (cruise, station, cast, sample), with unique associated columns. Eventually, the data frames get joined, and the event Core gets saved locally and in Google Drive. A visual diagram of the structure of the Bongo dataset is provided [here](https://lucid.app/lucidchart/71f5b051-7eae-4ad6-94b0-f4234037cb62/edit?beaconFlowId=3DD847BF3F2E52DA&page=0_0#). For the Event Core, most of the information for the unique layers can come from the `bongo_fnl` dataframe, whereas information pertaining specifically to the sample_ids will have to come from the `bongo` dataframe. 
...
bongo_fnl <- bongo %>%
  ...
  mutate(minimumDepthInMeters = 0,
         eventRemarks = ifelse(grepl("IYS2019-Kaganovsky-51-ND", bongo_fnl$eventID), "Bongo cancelled because of weather", ifelse(grepl("IYS2019-Kaganovsky-52-ND", bongo_fnl$eventID), "Bongo cancelled because of weather", NA))) %>%
  distinct(eventID, .keep_all = TRUE)

bongo_cast <- subset(bongo_fnl, decimalLatitude != "ND")
bongo_cast <- bongo_cast %>% 
  mutate(footprintWKT = paste("POINT"," (", bongo_cast$decimalLongitude, " ", bongo_cast$decimalLatitude, ")"),
         coordinateUncertaintyInMeters <- obistools::calculate_centroid(footprintWKT)[3])

bongo_fail <- subset(bongo_fnl, decimalLatitude == "ND")
bongo_fail$footprintWKT <- NA
bongo_fail$coordinateUncertaintyInMeters <- NA

bongo_fnl <- rbind(bongo_cast, bongo_fail) %>% 
...
write_csv(bongo_fnl, here("OBIS_data", "bongo2019_event.csv"))
```
...
Two Occurrence tables will be created, as we're dealing with two separate datasets created from the same data collection event. One dataset includes dry weight and length measurements, along with sex and lifestage, whereas the other dataset contains species-specific (estimated or expanded) abundance at each sampling station, with sex and lifestage. From my understanding, samples in Net 1 from the bongo sampling instrument were typically fixed as a whole for community and abundance analysis. Zooplankton in Net 2 was used predominantly for isotope or fatty acid analysis. 
...
Let's first create an Occurrence extension for the (estimated or expanded) abundance data. The abundance data file also includes data collected in 2020 so we do need to filter for the samples collected in 2019 first. 

``` {r bongo2019 abundance occ, eval = FALSE, warning = FALSE}
bongo2019_abun <- read_excel(here("original_data", "IYS zooplankton taxonomy data 20210115 (1).xlsx"), 
                  sheet = "All data") 

# Add additional data from Joanne Breckenridge. Create dummy columns for latitude and longitude to be populated later:
bongo_kag <- read_csv(here("original_data", "IYSKaganoSpecRemovedfromNet.csv")) %>%
  janitor::clean_names() %>%
  select(-c(year, yearletter, station_year))

bongo2019_abun <- rbind(bongo2019_abun, bongo_kag)

bongo2019_abun$date <- as.Date(bongo2019_abun$date)
bongo2019_abun <- subset(bongo2019_abun, date >= "2019-02-19" & date <= "2019-03-15") %>%
  mutate(net = 1,
         eventID = paste("IYS2019", "Kaganovsky", station, net, sep = "-"))
...
``` {r, eval = FALSE}
bongo2019_abun$identificationQualifier <- ifelse(grepl("sp.", bongo2019_abun$id), "sp. indet.", NA)
...
bongo2019_abun$scientificname <- gsub("Trochophore", "Animalia", bongo2019_abun$scientificname) 
# Unfortunately Trochophore (lifestage) can not be narrowed down further than 'Animalia' [as per Natalie M.]
...
Merge this dataset back into original dataframe. First remove the column "genus" from bongo2019_abun, because otherwise this one is copied. individualCount, ind_total, abund_m3_flow_est and abund_m3_depth_est are included in the original bongo2019_abun dataframe, because this will be the data frame that I work off of for the eMOF. 

``` {r, eval = FALSE}
bongo2019_abun <- bongo2019_abun %>% select(-genus)
bongo2019_abun <- left_join(bongo2019_abun, bongo2019_worms_id, by = "scientificname")
bongo2019_abun <- bongo2019_abun %>%
  select(eventID,
...
bongo2019_taxa_abundance <- bongo2019_abun %>% select(-c(individualCount, ind_total, abund_m3_flow_est, abund_m3_depth_est))

preservation <- bongo %>% select(eventID, preservation)
bongo2019_occ <- left_join(bongo2019_taxa_abundance, preservation, by = "eventID")

# Check for duplicated occurrenceIDs:
bongo2019_occ %>% janitor::get_dupes(occurrenceID) # Should be none!

# Make sure the folder path exists already (e.g. ./Bongo/tidy_data)
write_csv(bongo2019_occ, here("OBIS_data", "bongo2019_occ.csv"))

# Make sure the folder path exists already (e.g. ./Bongo/tidy_data)
#write_csv(bongo2019_taxa_abundance, here("Bongo", "tidy_data", "bongo2019_taxa_occ.csv"))
```

- [ ] To do: confirm that all unique species from the original dataset (IYS Zooplankton Taxonomy Abundance Data.xlsx - filtered for 2019) are represented in the bongo2019_taxa_abundance dataframe. 
